...
class VectorFloatTests: XCTestCase {
    ...
    func testDivision() {
        ...
        // infinity
        var vector5: VectorFloat = [0, 0, 0]
        vector5 /= 0
        XCTAssert(vector5[0].isNaN, "isNaN")
        
        // nan
        var vector6: VectorFloat = [1, 1, 1]
        vector6 /= 0
    ...
    func testDivisionInto() {
        ...
        var vector: VectorFloat = [1.0, 2.0, 4.0, 5.0]
        vector.inPlaceDivideIntoScalar(1)
        AssertVectorEqualWithAccuracy(vector, [1, 0.5, 0.25, 0.2])
        
        // in-place vector / vector
        var vector2: VectorFloat = [1, 0.1, 0.01, 0.001]
        vector2.inPlaceDivideIntoVector(vector)
        AssertVectorEqualWithAccuracy(vector2, [1, 5, 25, 200], accuracy: 1e-4)
        
        // out-of-place vector / vector
        let vector3: VectorFloat = [1000, 1000, 1000, 1000]
        let vector4 = vector2.divideIntoVector(vector3)
        AssertVectorEqualWithAccuracy(vector4, [1000, 200, 40, 5], accuracy: 1e-4)
        ...
        // infinity
        var vector6: VectorFloat = [0, 0, 0, 0]
        vector6.inPlaceDivideIntoScalar(1)
        XCTAssert(vector6[0].isInfinite, "isInfinite")
        
        // nan
        var vector7: VectorFloat = [0, 0, 0, 0]
